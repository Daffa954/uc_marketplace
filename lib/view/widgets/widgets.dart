import 'package:flutter/material.dart';
import 'package:go_router/go_router.dart';
import 'package:intl/intl.dart';
import 'package:provider/provider.dart';
import 'package:uc_marketplace/main.dart';
import 'package:uc_marketplace/model/model.dart';
import 'package:uc_marketplace/view/widgets/section_header.dart';
import 'package:uc_marketplace/viewmodel/auth_viewmodel.dart';
import 'package:uc_marketplace/viewmodel/chat_viewmodel.dart'; // Add this
import 'package:uc_marketplace/repository/restaurant_repository.dart'; // Add this
import 'package:uc_marketplace/viewmodel/home_viewmodel.dart';

part 'customcard.dart';
part 'customcardh.dart';
part 'preorderitemcard.dart';
part 'menuitemcard.dart';
part 'hero_section.dart';
part 'category_section.dart';
part 'food_cad.dart';
part 'popular_section.dart';
part 'home_app_bar.dart';
part 'menu_detail_page.dart';
