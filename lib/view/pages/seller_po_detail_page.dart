...
class _PreOrderDetailPageState extends State<PreOrderDetailPageD> {
  ...
  void initState() {
    super.initState();
    // Load detail (Pickup & Menu) saat halaman dibuka
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (widget.preOrder.preOrderId != null) {
        context.read<DetailViewModel>().loadPoDetails(widget.preOrder.preOrderId!);
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
                    ...
                    child: Column(
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: po.status == 'OPEN' ? Colors.green : Colors.red,
          ...
          // 2. Info Statistik (Kuota & Tanggal)
          SliverToBoxAdapter(
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  _infoItem(Icons.pie_chart, "Kuota", "${po.currentQuota} / ${po.targetQuota}"),
                  Container(width: 1, height: 40, color: Colors.grey[300]),
                  _infoItem(Icons.calendar_today, "Tutup PO", po.closeOrderDate ?? "-"),
                  Container(width: 1, height: 40, color: Colors.grey[300]),
          ...
          if (vm.isLoading)
            const SliverFillRemaining(
              child: Center(child: CircularProgressIndicator(color: Color(0xFFFF8C42))),
            )
          else ...[
            // 4. List Lokasi Pickup
            SliverToBoxAdapter(
              child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                child: const Text("Titik Penjemputan", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ),
            ),
            SliverList(
                ...
                (context, index) {
                  ...
                  return Container(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text("${pickup.date ?? ''} • ${pickup.startTime} - ${pickup.endTime}"),
                    ),
                  );
                },
            ...
            // 5. List Menu dalam PO Ini
            SliverToBoxAdapter(
              child: Padding(
                padding: const EdgeInsets.fromLTRB(16, 24, 16, 8),
                child: const Text("Menu Tersedia", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ),
            ),
            SliverList(
              delegate: SliverChildBuilderDelegate(
                (context, index) {
                  ...
                  // Format Harga
                  final price = NumberFormat.currency(locale: 'id_ID', symbol: 'Rp ', decimalDigits: 0).format(menu.price);
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(price, style: const TextStyle(color: Color(0xFFFF8C42), fontWeight: FontWeight.w600)),
                            ],
                          ),
                        ),
                      ],
